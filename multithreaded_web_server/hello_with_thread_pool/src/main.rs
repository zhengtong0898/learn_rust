use hello_with_thread_pool::ThreadPool;
use std::{
    fs,
...
fn main() {
    ...
    for stream in listener.incoming() {
        ...
        // 由于execute声明的是FnOnce, 所以函数体内使用到的外部变量, 全部都被move进闭包中了.
        // FnOnce 默认采取 move    策略(转移所有权)
        // FnMut  默认采用 Mut Ref 策略(可变引用)
        // Fn     默认采用 Ref     策略(不可变引用)
        pool.execute(|| {
